...
if(typeof(query) == 'object'){ // POST
	...
	adInform = {owner: query.body.upload_name, topic: query.body.upload_activity, shop: query.body.upload_shop, place: query.body.upload_location, discription: query.body.discription, start_time: query.body.start_time, end_time: query.body.end_time, type:query.body.type, path: query.file.path};
...
switch (param.page) {
	case 'homepage':
		...
		case 'getSurvey':
			...
			break;
		case 'askForUsrInform':
			usrsys.askForUsrInform(param.usr_id, function(result){output(JSON.stringify(result))})
			break;
		default:
